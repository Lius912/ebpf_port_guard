...
int hello(struct xdp_md *ctx) {
    ...
    if (pointer + sizeof(one_byte_pointer[0])*14 > ctx->data_end) {
        bpf_printk("too small1");
        return XDP_PASS;
    }
    ...
    if ((char *)my_ip + sizeof(struct my_ip_t) + sizeof(struct my_tcp_t) > ctx->data_end) {
    ...
    port_t port = (unsigned short) ((my_tcp2->src_port & 0x00FF) << 8) + ((my_tcp2->src_port & 0xFF00) >> 8);
    prog_id_t *value = bpf_map_lookup_elem(&local_port_prog_id_map, &port);
    if (!value) {
        // no port in map
        bpf_printk("port no in map");
    } else {
        prog_id_t prog_id = *value;
        struct prog_id_port_t prog_id_port = {prog_id, port};
        
        smallest_num_t* to_drop = bpf_map_lookup_elem(&prog_id_ports_map, &prog_id_port);
        if (!to_drop) {
            bpf_printk("no prog id port in map");
        } else {
            bpf_printk("DROPPED");
            return XDP_DROP;
        }
